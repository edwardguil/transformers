...
Attention::Attention(std::size_t in_dims, std::size_t n_heads, AttentionType type_)
    ...
    proj(register_module("proj", torch::nn::Linear(n_heads, in_dims))),
    type(type_) {}
...
torch::Tensor Attention::forward(torch::Tensor x, std::optional<torch::Tensor> context) {
    ...
    torch::Tensor output = proj(wei_sum);
...
EncoderBlock::EncoderBlock(std::size_t n_embeds, std::size_t n_heads)
    ...
    mlp(n_embeds),
    layer_norm1(register_module("layer_norm1", torch::nn::LayerNorm(std::vector<int64_t>{static_cast<int64_t>(n_embeds)}))),
    layer_norm2(register_module("layer_norm2", torch::nn::LayerNorm(std::vector<int64_t>{static_cast<int64_t>(n_embeds)}))) {}


torch::Tensor EncoderBlock::forward(torch::Tensor x) {
    torch::Tensor out = attention.forward(x);
    x = layer_norm1(x + out); // Add & Norm
    out = mlp.forward(x);
    x = layer_norm2(x + out); // Add & Norm
...
DecoderBlock::DecoderBlock(std::size_t n_embeds, std::size_t n_heads, bool decoderOnly_)
    ...
    layer_norm1(register_module("layer_norm1", torch::nn::LayerNorm(std::vector<int64_t>{static_cast<int64_t>(n_embeds)}))),
    layer_norm2(register_module("layer_norm2", torch::nn::LayerNorm(std::vector<int64_t>{static_cast<int64_t>(n_embeds)}))),
    layer_norm3(register_module("layer_norm3", torch::nn::LayerNorm(std::vector<int64_t>{static_cast<int64_t>(n_embeds)}))),
